const cardArray2 = document.getElementById("card_clone_array2");
...
function newCardFifa(group3, item3, content3) {
    for(let i = 0; i < content3.length; i++) {
        group3.innerHTML += item3(content3[i].linkToPage, content3[i].cardImg, content3[i].redTitle, content3[i].date, content3[i].title, content3[i].text)
    }
}

function cardFifa(linkToPage, cardImg, redTitle, date, title, text) {
    const card3 =  `<a href=${linkToPage} class="card">
...
newCardFifa(cardArray2, cardFifa, cardContent3)